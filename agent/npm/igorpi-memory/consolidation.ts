...
function findSimilarGroups(
  ...
  for (let i = 0; i < facts.length; i++) {
    ...
    if (groups.size >= maxGroups) {
      console.log(`[igorpi-memory] 🔄 Reached max groups limit (${maxGroups}), stopping early`);
      break;
    }
...
export function consolidateMemory(
  ...
  if (stats.sessionFacts < 100) {
    console.log(`[igorpi-memory] 🔄 Not enough records to consolidate (${stats.sessionFacts} < 100)`);
  ...
  if (facts.length < 100) {
    console.log(`[igorpi-memory] 🔄 Not enough records to consolidate (${facts.length} < 100)`);
    return { groupsFound: 0, recordsMerged: 0, recordsDeleted: 0 };
  }
  
  console.log(
    `[igorpi-memory] 🔄 Starting consolidation of ${facts.length} facts` +
  ...
  if (groups.size === 0) {
    console.log(`[igorpi-memory] 🔄 No similar groups found`);
    return { groupsFound: 0, recordsMerged: 0, recordsDeleted: 0 };
  }
  
  console.log(`[igorpi-memory] 🔄 Found ${groups.size} groups of similar records`);
  ...
  console.log(
    `[igorpi-memory] 🔄 Consolidation complete: ` +
