 ...
 * Вынесено из igorpi-subagents/session-handler.ts при разделении ответственности.
 */
...
const LAST_PURGE_KEY = Symbol.for("igorpi-memory:lastPurgeTimestamp");
...
export function registerSessionEvents(
  ...
  pi.on("session_start", async (event: any, ctx: any) => {
    ...
    if (sessionMemory) {
      ...
      console.log(`[igorpi-memory] 🧠 Session ID updated: ${newSessionId}, Project: ${projectRoot}`);
    }
    ...
    if (now - lastPurge > ONE_WEEK_MS) {
      ...
      if (stats.dbSizeMb > 50 || stats.toolOutputs > 5000) {
        console.log(`[igorpi-memory] 🧹 Running automatic purge (DB: ${stats.dbSizeMb.toFixed(1)} MB)...`);
        try {
          ...
          console.log(`[igorpi-memory] 🧹 Purged: ${d1} tools, ${d2} summaries, ${d3} keywords, ${d4} compressed. DB: ${stats.dbSizeMb.toFixed(1)} MB → ${sa.dbSizeMb.toFixed(1)} MB`);
          (globalThis as any)[LAST_PURGE_KEY] = now;
        } catch (err) { console.error(`[igorpi-memory] ❌ Purge failed:`, err); }
      }
    }
    ...
    if (stats.sessionFacts > 1000) {
      console.log(`[igorpi-memory] 🔄 Auto-consolidation triggered (${stats.sessionFacts} facts)`);
      try {
      ...
      } catch (err) { console.error(`[igorpi-memory] ❌ Auto-consolidation failed:`, err); }
    }
  });

  pi.on("session_shutdown", async () => {
    if (memoryDb) {
      try { memoryDb.close(); } catch (err) { console.error(`[igorpi-memory] ❌ Failed to close DB:`, err); }
