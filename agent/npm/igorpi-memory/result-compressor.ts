...
export async function compressResult(
  ...
  console.log(`[igorpi-memory] 🗜️ compressResult called: ${result.length} chars`);
  
  // 1. Не сжимаем маленькие результаты
  if (result.length < COMPRESSION_THRESHOLD) {
    console.log(`[igorpi-memory] 🗜️ Result too small (${result.length} < ${COMPRESSION_THRESHOLD} chars), skipping compression`);
    return {
      original: result,
      compressed: result,
      wasCompressed: false,
  ...
  if (cached) {
    console.log(`[igorpi-memory] 🗜️ Using cached compressed result for hash ${hash}`);
    return {
      original: result,
      compressed: cached,
      wasCompressed: true,
  ...
  console.log(`[igorpi-memory] 🗜️ Attempting LLM compression (${result.length} chars)...`);
  ...
  try {
    ...
    if (compressed && compressed.length > 0) {
      ...
      console.log(`[igorpi-memory] 🗜️ LLM compression successful: ${result.length} → ${compressed.length} chars`);
      ...
      console.log(`[igorpi-memory] 🗜️ LLM returned empty result, falling back to heuristic`);
    }
  } catch (err) {
    console.error(`[igorpi-memory] ❌ LLM compression failed: ${err instanceof Error ? err.message : String(err)}`);
    console.log(`[igorpi-memory] 🗜️ Falling back to heuristic compression`);
  }
  
  // 4. Fallback на эвристическое сжатие
  const fallback = heuristicCompress(result);
  memoryDb.saveCompressedResult(hash, fallback);
  console.log(`[igorpi-memory] 🗜️ Heuristic compression: ${result.length} → ${fallback.length} chars`);
...
Compressed summary:`;
  ...
  console.log(`[igorpi-memory] 🗜️ Calling LLM with prompt (${prompt.length} chars)...`);
  ...
  if (typeof (pi as any).chat === "function") {
    console.log(`[igorpi-memory] 🗜️ Using pi.chat() API`);
    try {
      ...
      console.error(`[igorpi-memory] ❌ pi.chat() failed: ${chatErr instanceof Error ? chatErr.message : String(chatErr)}`);
    }
  }
  
  // Метод 2: pi.models.generate() (если существует)
  if (!response && typeof (pi as any).models?.generate === "function") {
    console.log(`[igorpi-memory] 🗜️ Using pi.models.generate() API`);
    try {
      ...
      console.error(`[igorpi-memory] ❌ pi.models.generate() failed: ${genErr instanceof Error ? genErr.message : String(genErr)}`);
    }
  }
  
  // Метод 3: pi.exec() для вызова локальной модели через CLI (если существует)
  if (!response && typeof (pi as any).exec === "function") {
    console.log(`[igorpi-memory] 🗜️ Trying local model via pi.exec()`);
    try {
      ...
      console.error(`[igorpi-memory] ❌ pi.exec() failed: ${execErr instanceof Error ? execErr.message : String(execErr)}`);
    }
  }
  
  if (!response) {
    console.log(`[igorpi-memory] 🗜️ No LLM API available, returning empty`);
    return "";
  }
  
  const compressed = (response.text || response.content || "").trim();
  console.log(`[igorpi-memory] 🗜️ LLM response: ${compressed.length} chars`);
