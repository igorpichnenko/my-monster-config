/**
 * cpp-lsp.ts — Клиент для clangd Language Server
 * 
 * Использует clangd (настоящий LSP).
 * Требует compile_commands.json для работы.
 */
...
export class CppLanguageServer {
  ...
  async initialize(): Promise<void> {
    ...
    if (!this.lspPath) {
      throw new Error('clangd not found. Install: sudo apt install clangd');
    }

    // clangd использует stdio по умолчанию, флаг --stdio НЕ нужен
    ...
    this.process.stderr?.on("data", (data: Buffer) => {
      ...
      if (text) {
        console.error(`[igorpi-code-analysis] clangd stderr: ${text.slice(0, 200)}`);
      }
    });

    this.process.on("exit", (code) => {
      log(`clangd exited with code ${code}`);
    ...
    log(`✅ clangd LSP initialized for ${this.projectPath}`);
  }
