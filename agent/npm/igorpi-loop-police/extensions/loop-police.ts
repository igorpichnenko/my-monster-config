...
const CONFIG_PATH = join(EXT_DIR, "igorpi-loop-police.json");
...
const cfg: typeof DEFAULTS & { [key: string]: number } = (() => {
  if (!existsSync(CONFIG_PATH)) {
    try {
      ...
      console.error("[igorpi-loop-police] Failed to write default config:", err);
    }
    return { ...DEFAULTS };
  }
  try {
    ...
    console.error("[igorpi-loop-police] Failed to parse config, using defaults:", err);
    return { ...DEFAULTS };
  }
})();
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_start", (event: any, ctx: any) => {
    try {
      ...
      console.error("[igorpi-loop-police] Error in agent_start:", err);
    }
  });
  ...
  pi.on("agent_end", (event: any, ctx: any) => {
    try {
      ...
      console.error("[igorpi-loop-police] Error in agent_end:", err);
    }
  });
  ...
  pi.on("turn_start", (event: any, ctx: any) => {
    try {
      ...
      console.error("[igorpi-loop-police] Error in turn_start:", err);
    }
  });
  ...
  pi.on("message_update", (event: any, ctx: any) => {
    try {
      ...
      console.error("[igorpi-loop-police] Error in message_update:", err);
    }
  });
  ...
  pi.on("message_end", (event: any, ctx: any) => {
    try {
      ...
      if (state.thinkingAborted) {
        ...
        if (wasConsecutive) {
          label = "[CONSECUTIVE LOOP — truncated by igorpi-loop-police]";
          ...
          label = isSemantic
            ? "[SEMANTIC LOOP — truncated by igorpi-loop-police]"
            : "[THINKING LOOP — truncated by igorpi-loop-police]";
          advice = isSemantic
        ...
        pi.sendMessage(
          { customType: "igorpi-loop-police", content: advice, display: true },
          { triggerTurn: true }
        );
      ...
      console.error("[igorpi-loop-police] Error in message_end:", err);
    }
  });
    ...
    try {
      ...
      if (isReadTool(event.toolName)) {
        ...
        if (rawPath) {
          ...
          if (count >= limit) {
            ...
            return { block: true, reason: `igorpi-loop-police: file read ${count}x — ${cleanPath}:${start}:${end}` };
          }
        }
      }
      ...
      if (state.lastToolCallHash === currentHash) {
        ...
        if (state.repeatedToolCallCount >= cfg.REPEATED_TOOL_CALL_LIMIT) {
          ...
          return { block: true, reason: `igorpi-loop-police: repeated tool call ${state.repeatedToolCallCount}x` };
        }
      } else {
      ...
      if (cfg.ENABLE_TOOL_SEQUENCE_DETECTION) {
        if (state.toolLoopTriggered) {
          return { block: true, reason: "igorpi-loop-police: still in tool call loop" };
        }
        ...
        if (windowSize > 0) {
          ...
          return { block: true, reason: `igorpi-loop-police: ${windowSize}-call sequence repeating` };
        }
      ...
      console.error("[igorpi-loop-police] Error in tool_call:", err);
    }
  });

  pi.registerCommand("igorpi-loop-police", {
    description: "Show status; /igorpi-loop-police reset [all|<sessionId>]; /igorpi-loop-police set KEY=VAL [KEY=VAL ...]",
    handler: async (args: string, ctx: ExtensionCommandContext) => {
      try {
        ...
        console.error("[igorpi-loop-police] Error in command handler:", err);
        ctx.ui.notify(`Loop Police error: ${(err as Error).message}`, "error");
      }
    },
  });

  pi.registerMessageRenderer("igorpi-loop-police", (message, _opts, theme) => {
    try {
      return new Text(theme.fg("warning", String(message.content)), 0, 0);
    } catch (err) {
      console.error("[igorpi-loop-police] Error in message renderer:", err);
