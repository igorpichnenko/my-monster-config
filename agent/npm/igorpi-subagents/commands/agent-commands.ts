...
import { createActivityTracker } from "../helpers/activity-tracker.js";
...
export function registerAgentCommands(ctx: PiSubContext): void {
	...
	pi.registerCommand("agent-bg", {
		description:
			"Launch subagent in background: /agent-bg [--no-inject | --silent] <prompt>",
		handler: async (argStr, cmdCtx) => {
			...
			const modeNote = noInject
				? " (no-inject mode: result saved to DB only)"
				: "";
			cmdCtx.ui.notify(
				`Agent started: ${id}${modeNote}\nUse /agent-status ${id} to check progress.`,
				"info",
			);
		},
	});
	...
	pi.registerCommand("agent-steer", {
		description:
			"Send message to running subagent: /agent-steer <id> <message>",
		handler: async (argStr, cmdCtx) => {
			...
			if (record.status !== "running") {
				cmdCtx.ui.notify(
					`Agent is not running (status: ${record.status})`,
					"warning",
				);
				return;
			}
	...
	pi.registerCommand("agent-inject", {
		description:
			"Inject subagent result into parent context: /agent-inject <id>",
		handler: async (argStr, cmdCtx) => {
			const id = argStr.trim();
			if (!id) {
			...
			cmdCtx.ui.notify(
				`Result injected into parent context for agent ${id}`,
				"info",
			);
		},
	});
	...
	pi.registerCommand("agent-resume", {
		...
		handler: async (argStr, cmdCtx) => {
			...
			try {
				...
				cmdCtx.ui.notify(
					`Agent ${id} resumed with new task. Use /agent-status ${id} to check progress.`,
					"info",
				);
			} catch (err) {
				cmdCtx.ui.notify(
	...
	pi.registerCommand("agent-view", {
		...
		handler: async (argStr, cmdCtx) => {
			...
			cmdCtx.ui.notify(
				`Watching agent: ${agent.description} (press Ctrl+C to stop)`,
				"info",
			);
	...
	pi.registerCommand("agent-status", {
		...
		handler: async (argStr, cmdCtx) => {
			...
			if (activity?.activeTools && activity.activeTools.size > 0) {
				...
				for (const [_key, name] of activity.activeTools) {
					lines.push(`  - ${name}`);
				}
			}
