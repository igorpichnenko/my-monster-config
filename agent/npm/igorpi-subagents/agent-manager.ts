 ...
 *
 * v16: Добавлена поддержка интеграции с igorpi-code-analysis
 * - Сохранение pi и cwd для эмитта событий subagents:file_edited
 */
...
export class AgentManager {
	...
	// v16: Сохраняем pi для эмитта событий subagents:file_edited
	...
	spawn(
		...
		// v16: Сохраняем pi для последующего использования в resume
		if (!this.pi) {
			this.pi = pi;
		}
		...
		const record: AgentRecord = {
			...
			cwd: options.cwd ?? ctx.cwd, // ← v16: сохраняем cwd
		};
	...
	async resume(
		...
		try {
			const responseText = await resumeAgent(record.session, prompt, {
				// v16: Передаём pi, agentId, agentType, cwd для эмитта событий subagents:file_edited
				pi: this.pi,
				agentId: id,
