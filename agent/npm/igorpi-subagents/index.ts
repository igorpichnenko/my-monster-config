...
import { MemoryDatabase, getSessionMemory, resetSessionMemory, type SessionMemory } from "../igorpi-memory/index.js";
...
import { PiSubContext } from "./types/pi-sub-context.js";
...
export default function (pi: any) {
  ...
  try {
    ...
    console.log(`[igorpi-subagents] 🧠 Session memory initialized (Project: ${projectRoot})`);
  } catch (err) {
    console.error(`[igorpi-subagents] ❌ Failed to initialize session memory:`, err);
  }
  ...
  const manager = new AgentManager(
    ...
    (record) => {
      ...
      if (memoryDb && record.result) {
        ...
        } catch (err) { console.error(`[igorpi-subagents] ❌ Failed to save subagent result:`, err); }
      }
  ...
  if (!info) {
    console.warn(`[igorpi-subagents] ⚠️ onCompact called with undefined info for agent ${record.id}`);
    return;
  }
  ...
  if (memoryDb && info.summary && info.tokensBefore > 0) {
    try {
      ...
      console.error(`[igorpi-subagents] Failed to save compaction summary:`, err); 
    }
  }
