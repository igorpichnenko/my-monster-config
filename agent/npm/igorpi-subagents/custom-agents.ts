...
function loadFromDir(dir: string, agents: Map<string, AgentConfig>, source: "project" | "global"): void {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.error(`[igorpi-subagents] ⚠️ Failed to read agent file ${file}:`, err);
      continue;
    }
    ...
    try {
      ...
      console.error(
        `[igorpi-subagents] ⚠️ Failed to parse frontmatter in ${file}: ${err instanceof Error ? err.message : String(err)}. Skipping.`
      );
      continue;
    }
    ...
    if (!systemPrompt) {
      console.warn(`[igorpi-subagents] ⚠️ Agent ${name} in ${source} has empty system prompt. Skipping.`);
      continue;
    }
    ...
    try {
      ...
      console.error(
        `[igorpi-subagents] ⚠️ Failed to parse tools field in ${file}: ${err instanceof Error ? err.message : String(err)}. Using defaults.`
      );
