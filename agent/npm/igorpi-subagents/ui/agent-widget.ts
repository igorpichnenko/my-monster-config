...
/** Show the last N non-empty lines of text for activity display. */
function showLastLines(text: string, maxLines = 2, maxLineLen = 120): string {
	const lines = text.split("\n").filter((l) => l.trim());
	if (lines.length === 0) return "";
	const last = lines.slice(-maxLines);
	return last.map((l) => l.trim().slice(0, maxLineLen)).join("\n");
}

/** Truncate text to a single line, max `len` chars. */
function _truncateLine(text: string, len = 60): string {
	const line =
		text
			.split("\n")
			.find((l) => l.trim())
...
export function describeActivity(
	...
	// No tools active — show last lines of response text
	if (responseText && responseText.trim().length > 0) {
		const lastLines = showLastLines(responseText, 2, 120);
		return `${lastLines.replace(/\n/g, " · ")}…`;
	}
...
export class AgentWidget {
	...
	private renderFinishedLine(
		...
		const idTag = ` ${theme.fg("dim", a.id)}`;
		return `${icon} ${theme.fg("dim", name)}${modeTag}  ${theme.fg("dim", a.description)} ${theme.fg("dim", "·")} ${theme.fg("dim", parts.join(" · "))} ${idTag}${statusText}`;
	}
	...
	private renderWidget(tui: any, theme: Theme): string[] {
		...
		for (const a of running) {
			...
			runningLines.push([
				truncate(
					`${theme.fg("dim", "├─")} ${theme.fg("accent", frame)} ${theme.bold(name)}${modeTag}  ${theme.fg("muted", a.description)} ${theme.fg("dim", "·")} ${theme.fg("dim", statsText)}${idTag}`,
				),
