...
│   │   ├── igorpi-loop-police.json   # Конфигурация
│   │   └── igorpi-loop-police.ts     # Детектор зацикливания + анализ
│   └── node_modules/
│
...
|-------|--------|------------|
| `better-sqlite3` | ^12.11.1 | SQLite база данных |
| `@sinclair/typebox` | ^0.34.49 | Схемы параметров |
| `nanoid` | ^5.0.0 | Генерация ID |
...
| Команды igorpi-memory | ~12 (/memory-stats, /memory-search, /memory-add, /memory-purge, /memory-test, /memory-summaries, /memory-keywords, /memory-failures, /memory-subagents, /memory-consolidate + session events) |
| Команды igorpi-loop-police | 2 (/igorpi-loop-police, /igorpi-loop-police reset, /igorpi-loop-police set) |
| События igorpi-subagents | 8 (session_start, session_before_switch, session_shutdown, tool_execution_start, tool_result, turn_end, message_update, session_before_compact) |
| События igorpi-memory | 4 (session_start, session_shutdown, turn_end, session_before_compact) |
| События igorpi-loop-police | 5 (agent_start, turn_start, message_update, message_end, tool_call) |
| События igorpi-minimal-web | 4 (agent_start, tool_call×2, tool_result) |
| Фоновые процессы | 7 |
| Таблицы БД | 7 |
| FTS5 индексов | 7 |
| Репозиториев | 6 |
